...
"""
swcファイルから各糸球体のエリア（６０分割）のコンパートメント番号を抽出して、200/200_i_glomerular.datなどとして保存するプログラム
/200, /201というディレクトリをつくってファイルを保存するので予め'rm 20*'などで消しておく必要があることに注意
Usage:
$ python glomerular_extraction.py

by Buntaro Sakai 20181222
"""
import sys
import math
import numpy as np
...
#    shell_comps = [i for i in comps if shell(i)]  # extract compartment in the AL shell
    ...
    for i in range(len(shell_comps)):
        comp = shell_comps[i]
        theta = abs_yx_degree(comp[2], comp[3])
        phai = xz_degree(comp[2], comp[4])
        phais.append(phai)
        if theta < 20:
    ...
    os.mkdir(output_dir)
    for i in range(60):
        output_filename =  output_id + "/" + output_id + "_" + str(i).zfill(2) + "_glomrulars.dat"
        if len(comps_glomerulars[i]) != 0:
...
if __name__ == "__main__":
    swc_path = "../../../swc/717_registrated_v3.swc"
...
#    swc_path = "../../../swc/mirror050205_7_sn_bestrigid0106_mkRegion.swc"
    ...
    swc_path = "../../../swc/7171_registrated_v3.swc"
    output_id = "201"
    read_extract_save(swc_path, output_id)

    swc_path = "../../../swc/7172_registrated_v3.swc"
    output_id = "202"
    read_extract_save(swc_path, output_id)
