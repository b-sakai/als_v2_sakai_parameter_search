...
def mkMultipleStims_adaptation(spiketimef,dose,nstims,dose2,nstims2,parameter):
    ...
    de_para[0]= Calculate_Alpha1(dose,parameter)
    de_para[3]= Calculate_Alpha2(dose,parameter)
    de_para0_init = de_para[0]
    de_para3_init = de_para[3]

    for j in range(nstims):
        spike_t = [None for _ in range(500)]
        spike_t[0] = 6.0
        i=0
        de_para[0] = de_para0_init * orn_adaptation(j)
        de_para[3] = de_para3_init * orn_adaptation(j)
        #print de_para[0], de_para[3]
        while(i<150):
            if spike_t[i]>7.2:
                break
