...
def readdata_ln(file):
    with open(file, "r") as f:
        ...
        compsfloat = map(float, f.readline().split()[-4:])
        comps = map(int, compsfloat)

        rowfloat, columnfloat = map(float, f.readline().split())
        row, column = map(int, [rowfloat, columnfloat])
        lines = f.readlines()
...
if __name__ == "__main__":
    ...
    for file in fild_all_files(sys.argv[1]):
        ...
        if not '.dat' in file:
            continue
        print file
        cell_gid = file.split("/")[-1][:13]
        if cell_gid[7] == "3" and cell_gid[8] == "0":
            tstop, comps, data = readdata_ln(file)
            draw_PSTH_ln()
        elif cell_gid[7] == "2" and cell_gid[8] == "0":
